...
import { Scroll, LayoutGrid, FileText, Users, Settings, CreditCard, Menu, X, Bell, LogOut, ChevronRight } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2.5 px-4 py-5 border-b border-white/10">
        ...
        <span className="text-sm font-semibold text-white">ChangelogKit</span>
        {onClose && (
          <button onClick={onClose} className="ml-auto md:hidden text-gray-400 hover:text-white" aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        )}
      </div>
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} onClick={onClose}
              className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors group ${active ? 'text-emerald-300' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200'}`}
              ...
              {label}
              {active && <ChevronRight className="ml-auto w-3 h-3 text-emerald-400 opacity-60" />}
            </Link>
          )
        })}
      </nav>
      <div className="border-t border-white/10 px-3 py-3">
        <div className="flex items-center gap-3 px-2 py-2">
          <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold text-white" style={{ background: `linear-gradient(135deg, ${BRAND}, #0a7a56)` }}>U</div>
          ...
          <button className="text-gray-500 hover:text-gray-300 transition-colors p-1" aria-label="Sign out"><LogOut className="w-3.5 h-3.5" /></button>
        </div>
      </div>
    </div>
  )
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-950 overflow-hidden">
      <aside className="hidden md:flex flex-col w-60 flex-shrink-0 bg-gray-900/80 border-r border-white/[0.07]">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <header className="flex items-center h-14 px-4 border-b border-white/[0.07] bg-gray-950 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400">
            <span className="text-gray-600">ChangelogKit</span>
            <ChevronRight className="w-3 h-3 text-gray-700" />
            <span className="text-gray-200 font-medium">{currentLabel}</span>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <button className="relative w-8 h-8 flex items-center justify-center rounded-md text-gray-400 hover:text-gray-200 hover:bg-white/5" aria-label="Notifications">
